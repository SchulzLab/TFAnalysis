...
def main():
	parser=argparse.ArgumentParser(prog="CutTestData.py")
	...
	testTFs=["E2F1","FOXA2","FOXA1","CTCF","EGR1","GABPA","HNF4A","JUND","MAX","NANOG","REST","TAF1"]
	for iFiles in integratedFiles:
		for cFiles in cutInformationFiles:
			...
			if (cutTF in testTFs):
				...
				command=temp+" "+args.integratedData[0]+iFiles+" > "+args.outPath[0]+cutTF+"_Test_TEPIC_Annotation_Cut.txt"
				print(command)
				subprocess.call(command,shell=True)
